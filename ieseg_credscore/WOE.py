...
# Class Wrapper
class WOEEncoder:
    def __init__(self, pred_var, target_var, target_value=1):
    ...
    def test_limit(self, df, **kwargs):
        ...
        # case: continous variable -> convert to categorical
        if var_type in [float, int]:
            self.cont = True
            self.q = kwargs.get("q", 10)
            var_cat = self.cont2cat(df_copy[self.pred_var])
            df_copy[self.pred_var] = var_cat

            print(f"Detected '{var_type}' variable, automatically transformed using q={self.q} quantiles ... ")
        
        #stop limit stops aggregating bins, when the IV value will decrease more than specified value
        for stop_limit in [0.01, 0.025, 0.05, 0.1]:
            
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore")
                df_copy[self.pred_var] = df_copy[self.pred_var].astype("category")
                binning = woe_binning(df=df_copy, target_var=self.target_var, pred_var=self.pred_var, min_perc_total=0.05, min_perc_class=0.05, stop_limit=stop_limit, abbrev_fact_levels=50, event_class=self.target_value)

            iv_total = max(binning["iv_total_final"])

            binning = binning.groupby(["Group_2"]).agg(lambda x: list(x)).reset_index()
            binning["iv_total_final"] = binning["iv_total_final"].apply(lambda x: np.mean(x) if len(x) > 1 else x)

            n_bins = len([i for i in binning["Group_1"] if len(i) != 0])
            
            limit_list.append(stop_limit)
            iv_list.append(iv_total)
            n_bins_list.append(n_bins)
        
        output_df['stop_limit'] = limit_list
        output_df['iv_total'] = iv_list
        output_df['n_bins'] = n_bins_list
        
        return output_df
    # convert continous variable to categorical
    def cont2cat(self, var):
        return pd.qcut(var, q=self.q, labels=False, duplicates='drop').fillna(-1).astype(int).astype(str)
        
    # fit on training set
    def fit(self, df, stop_limit=0.05, **kwargs):
        df_copy = df.copy()
        ...
        if var_type in [float, int]:
            ...
            self.q = kwargs.get("q", 10)
            var_cat = self.cont2cat(df_copy[self.pred_var])
            concat = pd.concat([df_copy[self.pred_var], var_cat], axis=1)
        ...
        out_agg["name"] = self.pred_var + "_"
        out_agg["name"] += (out_agg.index + 1).astype(str)

        out_agg = out_agg[["name", "Group_1", "Group_2"]]

        # self.lookup = out_agg.copy()

        self.fit_dict = {}
        for _, row in out_agg.iterrows():
            for val in row["Group_1"]:
                self.fit_dict[val] = row["name"]

        if ("nan" not in self.fit_dict.keys()) & ("-1" in self.fit_dict.keys()):
            self.fit_dict["nan"] = self.fit_dict["-1"]
        if ("nan" not in self.fit_dict.keys()) & ("Missing" in self.fit_dict.keys()):
            self.fit_dict["nan"] = self.fit_dict["Missing"]

        print(f"Fitted {len(out_agg)} bins for '{self.pred_var}' (levels={len(self.fit_dict)})...")
...
# SOURCE: https://github.com/V1ad98/woeBinningPandas
...
def woe_binning_2 (df, target_var, pred_var, min_perc_total, min_perc_class, stop_limit, abbrev_fact_levels, bad, good):
    ...
    if len(dfrm.iloc[:,0].drop_duplicates()) == 2 and (dfrm.iloc[:,1].dtype in ['object', "category"])==True:
